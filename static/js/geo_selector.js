...
/**
 * GeoSelector — Widget cascade simple Haïti
 * Département → Commune → Section Communale
 *
 * Usage :
 *   new GeoSelector({
 *     deptSelect:    document.getElementById('id_departement'),
 *     communeSelect: document.getElementById('id_commune'),
 *     sectionSelect: document.getElementById('id_section'),  // optionnel
 *     baseUrl:       '/api/geo'
 *   });
 */
class GeoSelector {
  constructor({ deptSelect, communeSelect, sectionSelect, baseUrl = '/api/geo' }) {
    this.deptSelect    = deptSelect;
    this.communeSelect = communeSelect;
  ...
  async _init() {
    await this._loadDepartements();

    this.deptSelect.addEventListener('change', async () => {
      const slug = this.deptSelect.value;
      this._reset(this.communeSelect, 'Choisir une commune...');
      if (this.sectionSelect) this._reset(this.sectionSelect, 'Choisir une section...');
      if (slug) await this._loadCommunes(slug);
    });

    if (this.sectionSelect) {
      this.communeSelect.addEventListener('change', async () => {
        const communeNom = this.communeSelect.options[this.communeSelect.selectedIndex]?.text;
        const deptSlug   = this.deptSelect.value;
        this._reset(this.sectionSelect, 'Choisir une section...');
        if (communeNom && deptSlug) await this._loadSections(deptSlug, communeNom);
      });
    }
  }

  async _loadDepartements() {
    try {
      const res  = await fetch(`${this.baseUrl}/departements/`);
      const json = await res.json();
      if (!json.success) return;
      this._reset(this.deptSelect, 'Choisir un département...');
      json.data.forEach(dept => {
        const opt = document.createElement('option');
        opt.value = dept.slug;
        opt.textContent = dept.nom;
        this.deptSelect.appendChild(opt);
      });
    } catch (e) { console.error('GeoSelector: erreur départements', e); }
  }

  async _loadCommunes(deptSlug) {
    try {
      const res  = await fetch(`${this.baseUrl}/communes/?dept=${deptSlug}`);
      const json = await res.json();
      if (!json.success) return;
      this._reset(this.communeSelect, 'Choisir une commune...');
      this.communeSelect.disabled = false;
      json.data.forEach(commune => {
        const opt = document.createElement('option');
        opt.value = commune.nom;
        opt.textContent = commune.nom;
        opt.dataset.arrondissement = commune.arrondissement;
        this.communeSelect.appendChild(opt);
      });
    } catch (e) { console.error('GeoSelector: erreur communes', e); }
  }

  async _loadSections(deptSlug, communeNom) {
    try {
      const url  = `${this.baseUrl}/sections/?dept=${deptSlug}&commune=${encodeURIComponent(communeNom)}`;
      const res  = await fetch(url);
      const json = await res.json();
      if (!json.success) return;
      this._reset(this.sectionSelect, 'Choisir une section...');
      this.sectionSelect.disabled = false;
      json.data.sections_communales.forEach(section => {
        const opt = document.createElement('option');
        opt.value = section;
        opt.textContent = section;
        this.sectionSelect.appendChild(opt);
      });
    } catch (e) { console.error('GeoSelector: erreur sections', e); }
  }

  _reset(select, placeholder) {
    select.innerHTML = '';
    const opt = document.createElement('option');
    opt.value = '';
    opt.textContent = placeholder;
    select.appendChild(opt);
    select.disabled = select !== this.deptSelect;
  }
}
